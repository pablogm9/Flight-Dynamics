...
time_hour = np.array(time)
time_cg_hour = np.array(time_cg)
time_elevator_hour = np.array(time_elevator)

time = np.array([])
time_cg = np.array([])
time_elevator = np.array([])
...
for i in time_hour:
    ...
    second = int(i.split(':')[2])
    
    time = np.append(time,hour*3600+minute*60+second)
    
for i in time_cg_hour:
    ...
    time_cg = np.append(time_cg,hour*3600+minute*60+second)
    
for i in time_elevator_hour:
    ...
    time_elevator = np.append(time_elevator,hour*3600+minute*60+second)    

weights = np.array(np.concatenate((W, W_elevator,W_cg*g_0)),dtype=np.float)
times = np.concatenate((time,time_elevator,time_cg))

z = np.polyfit(times,weights,1)
time_range = np.arange(1000,3200,1)
poly = np.poly1d(z)
weight_range = poly(time_range)

plt.scatter(time,W/1000,label=r'$C_L$ vs. $C_D$ series',marker='D',color='g',zorder=2,s=95,facecolors='None')
plt.scatter(time_elevator,W_elevator/1000,label=r'Elevator series',marker='^',color='b',zorder=2,s=95,facecolors='None')
plt.scatter(time_cg,W_cg*g_0/1000,label=r'Shift in $x_{cg}$ series',marker='o',color='r',zorder=2,s=95,facecolors='None')
plt.plot(time_range,weight_range/1000,'k--',label='Linear LSE regression',zorder=1)

plt.xlabel(r'Time, $t$ [sec]',fontsize=13)
plt.ylabel(r'Aircraft weight [kN]',fontsize=13)
plt.title(r'Computed aircraft weight over time',fontsize=17,pad=15)
