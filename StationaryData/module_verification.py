...
#C_L_new = np.array(2*W/(0.5*rho*(V_TAS**2)*S),dtype=np.float)


# ------------- Compute C_D -------------
C_D = np.array(thrust_total/(0.5*rho*(V_TAS**2)*S),dtype=np.float)
C_D_new = np.array(2*thrust_total/(0.5*rho*(V_TAS**2)*S),dtype=np.float)
...
z_new = np.polyfit(C_L_squared,C_D_new,1) #Coefficients, highest power first

# C_D_0 and e
e_new = 1/(np.pi*A*z_new[0])
C_D_0_new = z_new[1]
...
'''
C_L_alpha_coefficients_new = np.polyfit(np.array(a,dtype=np.float),C_L_new,1)
C_L_alpha_polynomial_new = np.poly1d(C_L_alpha_coefficients_new)
'''
...
#C_L_range_new = C_L_alpha_polynomial_new(alpha_range)
...
alpha_0_new = alpha_range[idx][0]


'''
# Slope of C_L vs alpha curve
C_L_alpha_new = C_L_alpha_coefficients_new[0]

# Alpha_0 
idx_new = np.where(np.abs(C_L_range_new)==np.abs(C_L_range_new).min())
alpha_0_new = alpha_range[idx_new][0]
'''
...
C_D_range_new = C_D_0_new + (1/(np.pi*A*e_new))*(C_L_range**2) 



'''
C_D_range_new = C_D_0 + (1/(np.pi*A*e))*(C_L_range_new**2)
C_D_new = C_D_0 + (1/(np.pi*A*e))*(C_L_new**2)
'''
...
thrust_new = thrust_total_elevator/5

T_c_new = np.array([])

for i in range(len(thrust_total_elevator)):
    T_c_i = thrust_new[i] / (rho_elevator[i] * (V_TAS_elevator[i] ** 2) * (D ** 2))
    T_c_new = np.append(T_c_new, T_c_i)
...
delta_e_new = np.array([])

for i in range(len(standard_thrust)):
    b = delta_e[i] - ((1 / C_m_delta) * C_m_T_c * (T_c_s[i] - T_c_new[i]))
    delta_e_new = np.append(delta_e_new, b)
...
v_new = np.polyfit(np.array(a_elevator,dtype=np.float),np.array(delta_e_new,dtype=np.float),1) #Coefficients, highest power first

elevator_alpha_slope_new = v_new[0]
elevator_alpha_intercept_new = v_new[1]
C_m_alpha_new = elevator_alpha_slope_new*-C_m_delta
...
plt.plot(alpha_range,C_L_range,'r--',label=r'Original C$_L$ vs. $\alpha$ curve',zorder=1)
plt.scatter(angle_of_attack,C_L,label='Original data points',marker='^',color='g',zorder=2,s=75,facecolors='None')

plt.scatter(angle_of_attack,C_L,label='New data points',marker='o',color='g',zorder=2,s=85,facecolors='None')

plt.scatter(alpha_0,C_L_range[idx],label=r'Zero-lift angle of attack $\alpha_0$',marker='o',zorder=3,s=75,color='b')
...
plt.plot(alpha_range,C_D_range,'r--',label=r'Original C$_D$ vs. $\alpha$ curve',zorder=1)
plt.scatter(angle_of_attack,C_D,label='Original data points',marker='^',color='g',zorder=2,s=75,facecolors='None')

plt.plot(alpha_range,C_D_range_new,'r-.',label=r'New C$_D$ vs. $\alpha$ curve',zorder=1)
plt.scatter(angle_of_attack,C_D_new,label='New data points',marker='o',color='g',zorder=2,s=85,facecolors='None')

plt.scatter(alpha_0,C_D_range[idx],marker='o',label=r'Original zero-lift drag coefficient, $C_{D_0}$',zorder=3,s=75,color='b')
plt.scatter(alpha_0_new,C_D_range_new[idx],marker='x',label=r'New zero-lift drag coefficient, $C_{D_0}$',zorder=3,s=85,color='b')
...
plt.plot(C_D_range,C_L_range,'r--',label=r'Original C$_L$ vs. C$_D$ curve',zorder=1)
plt.scatter(C_D,C_L,label='Original data points',marker='^',color='g',zorder=2,s=75,facecolors='None')


plt.plot(C_D_range_new,C_L_range,'r-.',label=r'New C$_L$ vs. C$_D$ curve',zorder=1)
plt.scatter(C_D_new,C_L,label='New data points',marker='o',color='g',zorder=2,s=85,facecolors='None')


plt.scatter(C_D_range[idx],C_L_range[idx],marker='o',label=r'Original zero-lift drag coefficient, $C_{D_0}$',zorder=3,s=75,color='b')
plt.scatter(C_D_range_new[idx],C_L_range[idx],marker='x',label=r'New zero-lift drag coefficient, $C_{D_0}$',zorder=3,s=85,color='b')
...
delta_range_new = elevator_alpha_intercept_new + elevator_alpha_slope_new*alpha_range

plt.plot(alpha_range,delta_range,'r-',label=r'Original trim curve',zorder=1)
plt.scatter(np.array(a_elevator,dtype=np.float),delta_e_reduced,label='Original data points',marker='^',color='g',zorder=2,s=75,facecolors='None')

plt.plot(alpha_range,delta_range_new,'r-.',label=r'New trim curve',zorder=1)
plt.scatter(np.array(a_elevator,dtype=np.float),delta_e_new,label='New data points',marker='o',color='g',zorder=2,s=85,facecolors='None')
...
w_new = np.polyfit((1/(V_EAS_reduced**2)),delta_e_new,1)

V_range = np.linspace(V_stall,110,10000)
delta_V_range = (1/(V_range**2))*w[0] + w[1]
delta_V_range_new = (1/(V_range**2))*w_new[0] + w_new[1]


plt.plot(V_range,delta_V_range,'r-',label=r'Original trim curve',zorder=1)
plt.scatter(V_EAS_reduced,delta_e_reduced,label='Original data points',marker='^',color='g',zorder=2,s=75,facecolors='None')

plt.plot(V_range,delta_V_range_new,'r-.',label=r'New trim curve',zorder=1)
plt.scatter(V_EAS_reduced,delta_e_new,label='New data points',marker='o',color='g',zorder=2,s=85,facecolors='None')
